...
class BWT(object):
    ...
    def __init__(self, aligner, include_wildcard, include_baits, read_one, read_two, threads, output_file, debug, clean, local_database, mapq, mapped, coverage, include_other_models):
        ...
        if self.include_baits == True:
            ...
            self.index_directory_bowtie2_baits = os.path.join(
                self.db, self.indecies_directory, "baits_reference", "{}".format("bowtie2_baits"))
            self.index_directory_bwa_baits = os.path.join(
                self.db, self.indecies_directory, "baits_reference", "{}".format("bwa_baits"))
        ...
        if self.include_wildcard == True and self.include_other_models == False:
            ...
            self.index_directory_bowtie2 = os.path.join(
                self.db, self.indecies_directory, "card_wildcard_reference", "{}".format("bowtie2"))
            self.index_directory_kma = os.path.join(
                self.db, self.indecies_directory, "card_wildcard_reference", "{}".format("kma"))
            self.index_directory_bwa = os.path.join(
                self.db, self.indecies_directory, "card_wildcard_reference", "{}".format("bwa"))
            ...
            self.reference_genome = os.path.join(
                self.data, "card_reference_all.fasta")
            self.index_directory_bowtie2 = os.path.join(
                self.db, self.indecies_directory, "card_reference_all", "{}".format("bowtie2"))
            self.index_directory_kma = os.path.join(
                self.db, self.indecies_directory, "card_reference_all", "{}".format("kma"))
            self.index_directory_bwa = os.path.join(
                self.db, self.indecies_directory, "card_reference_all", "{}".format("bwa"))
            ...
            self.index_directory_bowtie2 = os.path.join(
                self.db, self.indecies_directory, "card_wildcard_reference_all", "{}".format("bowtie2"))
            self.index_directory_kma = os.path.join(
                self.db, self.indecies_directory, "card_wildcard_reference_all", "{}".format("kma"))
            self.index_directory_bwa = os.path.join(
                self.db, self.indecies_directory, "card_wildcard_reference_all", "{}".format("bwa"))
        else:
            logger.info("CARD[homolog]")
            self.reference_genome = os.path.join(
                self.data, "card_reference.fasta")
            self.index_directory_bowtie2 = os.path.join(
                self.db, self.indecies_directory, "card_reference", "{}".format("bowtie2"))
            self.index_directory_kma = os.path.join(
        ...
        self.mapping_overall_stats = os.path.join(
            self.working_directory, "{}.overall_mapping_stats.txt".format(self.output_file))
        self.mapping_artifacts_stats = os.path.join(
            self.working_directory, "{}.artifacts_mapping_stats.txt".format(self.output_file))
        self.mapping_reference_stats = os.path.join(
            self.working_directory, "{}.reference_mapping_stats.txt".format(self.output_file))
        self.mapping_baits_stats = os.path.join(
            self.working_directory, "{}.baits_mapping_stats.txt".format(self.output_file))

        self.baits_reads_count = os.path.join(
            self.working_directory, "{}.baits_reads_count.temp.txt".format(self.output_file))
        self.reads_baits_count = os.path.join(
            self.working_directory, "{}.reads_baits_count.temp.txt".format(self.output_file))
        self.aro_term_reads = os.path.join(
            self.working_directory, "{}.aro_term_reads.temp.txt".format(self.output_file))
        ...
        self.output_tab_coverage_all_positions = os.path.join(
            self.working_directory, "{}.coverage_all_positions.temp.txt".format(self.output_file))
        self.output_tab_coverage_all_positions_summary = os.path.join(
            self.working_directory, "{}.coverage_all_positions.summary.temp.txt".format(self.output_file))
        self.model_species_data_type = os.path.join(
            self.working_directory, "{}.model_species_data_type.temp.txt".format(self.output_file))
        self.allele_mapping_data_json = os.path.join(
            self.working_directory, "{}.allele_mapping_data.json".format(self.output_file))
        self.allele_mapping_data_tab = os.path.join(
            self.working_directory, "{}.allele_mapping_data.txt".format(self.output_file))
        ...
        self.baits_mapping_data_tab = os.path.join(
            self.working_directory, "{}.baits_mapping_data.temp.txt".format(self.output_file))
        self.baits_mapping_data_json = os.path.join(
            self.working_directory, "{}.baits_mapping_data.temp.json".format(self.output_file))
        self.reads_mapping_data_json = os.path.join(
            self.working_directory, "{}.reads_mapping_data.temp.json".format(self.output_file))
        ...
        self.baits_card_data_tab = os.path.join(
            self.working_directory, "{}.baits_card_data.temp.txt".format(self.output_file))
        self.card_baits_reads_count_json = os.path.join(
            self.working_directory, "{}.card_baits_reads_count.temp.json".format(self.output_file))
    ...
    def align_bowtie2_baits_to_genes(self, reference_genome, index_directory, output_sam_file):
        ...
        logger.info("align baits -f {} to complete genes in {}".format(
    ...
    def get_coverage_all_positions(self):
        ...
        output_tab = self.output_tab_coverage_all_positions
        subprocess.run(
            f"bedtools genomecov -ibam {sorted_bam_file} > {output_tab}", shell=True, check=True)
        ...
        output_file = self.output_tab_coverage_all_positions_summary
        subprocess.run(
            f"cat {output_tab} | awk '$2 > 0' | cut -f1,3,4,5 > {output_file}", shell=True, check=True)
            ...
            if i.isdigit():
                ...
                for c in data[i]["ARO_category"]:
                    ...
                    if data[i]["ARO_category"][c]["category_aro_name"] not in categories[data[i]["ARO_category"][c]["category_aro_class_name"]]:
                        categories[data[i]["ARO_category"][c]["category_aro_class_name"]].append(
    ...
    def get_variant_details(self):
        ...
        df.to_csv(filename=self.model_species_data_type, single_file=True,
                  mode="w", sep="\t", header_first_partition_only=True, index=False)
    ...
    def get_coverage_details(self, hit_id):
        ...
        with open(self.output_tab_coverage_all_positions_summary, 'r') as csvfile:
            ...
            for row in reader:
                if hit_id == row[0]:
                    sequences[hit_id]["covered"] = sequences[hit_id]["covered"] + \
                        int(row[1])
                    sequences[hit_id]["length"] = int(row[2])
        sequences[hit_id]["uncovered"] = sequences[hit_id]["length"] - \
            sequences[hit_id]["covered"]
    ...
    def filter_count_reads(self, is_mapped="false", length=""):
        ...
        """
        read_one = os.path.join(self.working_directory,
                                "{}.R1.fastq".format(self.output_file))
        read_two = os.path.join(self.working_directory,
                                "{}.R2.fastq".format(self.output_file))
        options = ""
    ...
    def probes_stats(self, baits_card):
        ...
        with open(self.mapping_baits_stats, "w") as tab_out:
            ...
            for i in stats:
                accession, baits_count, baits_with_reads_count, reads_count, sample = self.get_counts(
                    i, data_out)
    ...
    def get_model_id(self, models_by_accession, alignment_hit):
        ...
        if alignment_hit[0:22] == "Prevalence_Sequence_ID" or alignment_hit[0:4] == "ARO:":
            ...
            try:
                ...
                logger.warning(
                    "missing aro accession: {} for alignment {} -> {}".format(accession, alignment_hit, e))
        return model_id
    ...
    def summary(self, alignment_hit, models, variants, baits, reads, models_by_accession, mutation, read_coverage, consensus_sequence):
        ...
        try:
            ...
            if "Prevalence_Sequence_ID" in alignment_hit:
                ...
                if model_id in variants.keys():
                    ...
                    for s in variants[model_id]:
                        prevalence_sequence_id = alignment_hit.split("|")[
                            0].split(":")[-1]
                        ...
                        for accession in variants[model_id][prevalence_sequence_id]:
                            ...
                            if variants[model_id][prevalence_sequence_id][accession]["data_type"] not in observed_data_types:
                                observed_data_types.append(
                                    variants[model_id][prevalence_sequence_id][accession]["data_type"])
                            ...
                            if variants[model_id][prevalence_sequence_id][accession]["species_name"] not in observed_in_pathogens:
                                observed_in_pathogens.append(
                                    variants[model_id][prevalence_sequence_id][accession]["species_name"].replace('"', ""))
                        ...
                        if "Prevalence_Sequence_ID" in alignment_hit:
                            ...
                            try:
                                ...
                                percent_identity = float(
                                    variants[model_id][prevalence_sequence_id][_accession]["percent_identity"])
                            except Exception as e:
                                reference_allele_source = ""
                                # logger.debug(alignment_hit)
                                # logger.debug(json.dumps(alignments, indent=2))
                                # logger.debug(json.dumps(variants[model_id], indent=2))
                                logger.warning("missing key with Prev_id: {}, Exception: {}, Database: {} for model_id: {}".format(
            ...
            number_of_mapped_baits, number_of_mapped_baits_with_reads, average_bait_coverage, bait_coverage_coefficient_of_variation = self.baits_reads_counts(
                models[model_id]["ARO_accession"])
            ...
            if self.aligner == "kma":
                try:
                    ...
                    if trailing_bases:
                        input_seq = ''.join([input_seq, 'NN']) if trailing_bases == 1 else ''.join(
                            [input_seq, 'N'])
                    ...
                    logger.warning(
                        "model with id : {}, has few mapped reads to make consensus sequence skipping: {}".format(model_id, e))
                    return {}
    ...
    def get_summary(self):
        ...
        for alignment_hit in reads.keys():
            jobs.append((alignment_hit, models, variants, baits, reads,
                        models_by_accession, mutation, read_coverage, consensus_sequence,))
                ...
                if r:
                    writer.writerow([
                        ...
                        "; ".join(r["resistomes"]["Resistance Mechanism"]
                                  ), r["depth"], r["snps"], r["consensus_sequence_dna"], r["consensus_sequence_protein"]
                    ])
        ...
        for r in summary:
            if r:
                ...
                if r[index] not in mapping_summary.keys():
                    ...
                    mapping_summary[r[index]]["cvterm_name"].append(
                        r["cvterm_name"])
                    mapping_summary[r[index]]["aro_accession"].append(
                        r["aro_accession"])
                    mapping_summary[r[index]]["model_type"].append(
                        r["model_type"])
                    ...
                    for p in r["observed_in_pathogens"]:
                        mapping_summary[r[index]
                                        ]["observed_in_pathogens"].append(p)

                    mapping_summary[r[index]]["mapped"].append(
                        r["reads"]["mapped"])
                    mapping_summary[r[index]]["range_of_reference_allele_source"].append(
                        r["range_of_reference_allele_source"])
                    mapping_summary[r[index]]["unmapped"].append(
                        r["reads"]["unmapped"])
                    ...
                    mapping_summary[r[index]]["length_coverage"].append(
                        r["length_coverage"]["covered"])
                    mapping_summary[r[index]]["mapq_average"].append(
                        r["mapq_average"])
                    mapping_summary[r[index]]["reference_sequence_length"].append(
                        r["reference"]["sequence_length"])
                    mapping_summary[r[index]]["number_of_mapped_baits"].append(
                        r["number_of_mapped_baits"])
                    mapping_summary[r[index]]["number_of_mapped_baits_with_reads"].append(
                        r["number_of_mapped_baits_with_reads"])
                    mapping_summary[r[index]]["average_bait_coverage"].append(
                        r["average_bait_coverage"])
                    mapping_summary[r[index]]["bait_coverage_coefficient_of_variation"].append(
                        r["bait_coverage_coefficient_of_variation"])
                    ...
                    if r["database"] not in mapping_summary[r[index]]["database"]:
                        mapping_summary[r[index]]["database"].append(
                            r["database"])
        ...
        with open(self.gene_mapping_data_tab, "w") as tab_out:
            ...
            for i in mapping_summary:
                ...
                if len(mapping_summary[i]["percent_coverage"]) > 0:
                    average_percent_coverage = sum(map(
                        float, mapping_summary[i]["percent_coverage"]))/len(mapping_summary[i]["percent_coverage"])

                if len(mapping_summary[i]["length_coverage"]) > 0:
                    average_length_coverage = sum(map(
                        float, mapping_summary[i]["length_coverage"]))/len(mapping_summary[i]["length_coverage"])

                if len(mapping_summary[i]["mapq_average"]) > 0:
                    average_mapq = sum(map(
                        float, mapping_summary[i]["mapq_average"]))/len(mapping_summary[i]["mapq_average"])

                mate_pairs = []
                mp = {item: mapping_summary[i]["mate_pair"].count(
                    item) for item in mapping_summary[i]["mate_pair"]}
                for k in mp:
                    if k != i.replace(" ", "_"):
                ...
                if self.include_other_models is False:
                    ...
                    if "protein homolog model" in mapping_summary[i]["model_type"]:
                        ...
                        writer.writerow([
                            ...
                            observed_in_plasmids,
                            "; ".join(
                                mapping_summary[i]["observed_in_pathogens"]),
                            format(
                            ...
                            "; ".join(
                                mapping_summary[i]["AMR Gene Family"]),
                            "; ".join(mapping_summary[i]["Drug Class"]),
                            "; ".join(
                                mapping_summary[i]["Resistance Mechanism"])
                        ])
    ...
    def check_index(self, index_directory, reference_genome):
        ...
        if self.aligner == "bowtie2":
            files = [os.path.basename(x) for x in glob.glob(
                os.path.join(os.path.dirname(index_directory), "*"))]
            ...
            if (("bowtie2.1.bt2" in files) and
                ...
                logger.info("create index for reference: {} using aligner: {} ".format(
                    reference_genome, self.aligner))
                self.create_index(index_directory=index_directory,
                                  reference_genome=reference_genome)
            else:
                logger.info("index already exists for reference: {} using aligner: {}".format(
                    reference_genome, self.aligner))
        elif self.aligner == "kma":
            files = [os.path.basename(x) for x in glob.glob(
                os.path.join(os.path.dirname(index_directory), "*"))]
    ...
    # check index / create index / align
        ...
        if self.aligner == "bowtie2":
            if self.read_two == None:
                self.align_bowtie2_unpaired(reference_genome=self.reference_genome,
...
#        logger.info("only extract alignment of specific length")
        ...
        if self.include_baits == True:
            ...
            self.align_bowtie2(reference_genome=self.reference_genome_baits,
                               index_directory=self.index_directory_bowtie2_baits, output_sam_file=self.output_sam_file_baits)

            logger.info("convert SAM file to BAM file")
            self.convert_sam_to_bam(
                input_sam_file=self.output_sam_file_baits, output_bam_file=self.output_bam_file_baits)
        ...
        self.clean_files()
        logger.info("Total Time: {} seconds".format(
            format(time.time() - t0, '.3f')))
        logger.info("Done.")
